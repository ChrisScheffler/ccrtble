  ...
  _resolveCharacteristics() {
    return timeout(10000, async (resolve, reject) => {
      try {
        ...
        this._service = (await this._peripheral.discoverServicesAsync([UUID_SERVICE_CCRTBLE])).find(entry => entry.uuid === UUID_SERVICE_CCRTBLE);
        if (this._service) {
          let chars = await this._service.discoverCharacteristicsAsync([UUID_CHARACTERISTIC_DATA, UUID_CHARACTERISTIC_COMMAND]);
          this._dataCharacteristic = chars.find(entry => entry.uuid === UUID_CHARACTERISTIC_DATA);
          this._commandCharacteristic = chars.find(entry => entry.uuid === UUID_CHARACTERISTIC_COMMAND);
          if (this._dataCharacteristic && this._commandCharacteristic) {
            resolve();
          }
        }
        reject(new Error("service or char missing"));
      } catch (error) {
        return reject(error);
      }
    });
